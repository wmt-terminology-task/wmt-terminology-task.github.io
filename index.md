...
Participants in this task will be provided with source text and two dictionaries: (1) a `segment-level terminology dictionary` and (2) a  `segment-level randomly sampled translation dictionary`.
...
## Training Data
You can download the traninig data from [here](https://drive.google.com/drive/folders/1dwaX0HANYAPGtfNYQqsRbc45Mab_CdV-?usp=share_link).
A development set is also included within this resource.
Please read the `README.md` in this folder for more information.

``IMPORTANT❗``: The use of any additional data beyond that provided in these resources is strictly prohibited.
